import { LinkRulesValidator, NotSameInterfaceInput, NotSameInterfaceNode, NotSameInterfaceType, SameNodeVariableType } from '../src/types/LinkRules/LinkRule';
import { NodeComponentFactory } from '../src/types/factory/NodeComponentFactory';
import { expect, test } from 'vitest'
import { NodeComponentState, VariableNodeComponent, NodeType, VariableType } from '../src/types/NodeComponent';
...
test('OK - validate interfaces linking of different type: I1 - O2', () => {
    ...
    if (inputOne && outputTwo) {
        const { allValid, successfullRules, failedRules } = linkRuleValidator.validateRules(inputOne, outputTwo)
...
test('OK - validate interfaces linking of different type: I2 - O1', () => {
    ...
    if (inputTwo && outputOne) {
        const { allValid, successfullRules, failedRules } = linkRuleValidator.validateRules(inputTwo, outputOne)
        expect(allValid).toBeTruthy()
    }
})
...
test('KO - validate interfaces linking of the same type: I1 - I2', () => {
    ...
    if (nodeOneInputInterface && nodeTwoInputInterface) {
        const { allValid, successfullRules, failedRules } = linkRuleValidator.validateRules(nodeOneInputInterface, nodeTwoInputInterface)
...
test('KO - validate interfaces linking of the same type: O2 - O1', () => {
    ...
    if (nodeTwoOutputInterface && nodeOneOutputInterface) {
        const { allValid, successfullRules, failedRules } = linkRuleValidator.validateRules(nodeTwoOutputInterface, nodeOneOutputInterface)
        expect(allValid).toBeFalsy()
    }
})
...
test('KO - validate interfaces linking of the same node with different interface type', () => {
    ...
    if (nodeOneInputInterface && nodeOneOutputInterface) {
        const { allValid, successfullRules, failedRules } = linkRuleValidator.validateRules(nodeOneInputInterface, nodeOneOutputInterface)
        expect(allValid).toBeFalsy()
    }
...
test('KO - validate interfaces linking of different node variable type', () => {
    ...
    const { allValid } = linkRuleValidator.validateRules(nodeOneInputInterface, nodeOneOutputInterface)
    ...
    const { allValid: allValid2 } = linkRuleValidator.validateRules(nodeOneInputInterface, nodeOneOutputInterface)
